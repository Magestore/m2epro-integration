...
class M2eOrderStart extends \Magestore\M2eIntegration\Observer\M2esuccess
    implements ObserverInterface
{
    ...
    {
        $m2eorder = $observer->getEvent()->getOrder();
        try{
            if ($this->isM2eProActive()) {
                if (!$this->_coreRegistry->registry('os_m2e_order_create_before')) {
                    $this->_coreRegistry->register('os_m2e_order_create_before',$m2eorder);
                }
            }
        }catch(\Exception $e) {
            /* log issue */
            $this->logger->log($e->getMessage(), 'm2eOrderStart');
        }
    }
}